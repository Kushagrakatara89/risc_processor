// ============================================================================
// Pipelined Processor — Top Module (Simplified)
// Pipeline: IF -> ID -> RR -> EX -> MM -> WB
// Instructions: ADI, ADD, ADC, ADZ, NDU, LHI, LW, SW, BEQ, JAL, JRI
// Hazards: Load-use stall only (no branch prediction, no forwarding)
// ============================================================================
...
module pipelined_processor (
    ...
    // Gate write enables when stalling due to load-use
    wire RR_Wr_En_gated   = nop_inject ? 1'b0 : RR_Wr_En;
    wire MEM_Wr_En_gated  = nop_inject ? 1'b0 : MEM_Wr_En;

    assign control_signals = {RR_A3_sel, RR_Wr_En_gated, ALU_Src,
                               ALU_Oper, Reg_D3_Sel, MEM_Wr_En_gated};
    ...
    assign ctrl_rr = ctrl_from_id_rr;
    ...
    mux_21 #(.IN_WIDTH(3),.OUT_WIDTH(3)) mux_a2 (
        .A(ir_from_id_rr[8:6]), .B(ir_from_id_rr[11:9]),
    ...
    RR2EX_Pipline_Reg rr_ex_reg (
        ...
        .instr_in(ir_from_id_rr), .cntrl_in(ctrl_rr),
    ...
    // ALU source A = RF_D1, source B = RF_D2 or imm6
    assign alu_src_A = RF_D1_from_rr_ex;
    ...
    mux_41 #(.IN_WIDTH(16),.OUT_WIDTH(16)) alu_srcb_mux (
        .A(RF_D2_from_rr_ex), .B(imm6), .C(lhi_data), .D(16'd0),
    ...
    mux_41 #(.IN_WIDTH(3),.OUT_WIDTH(3)) rd_addr_mux (
        .A(ir_from_rr_ex[5:3]), .B(ir_from_rr_ex[8:6]),
    ...
    mux_21 #(.IN_WIDTH(16),.OUT_WIDTH(16)) lhi_mux (
        .A(alu_out), .B(lhi_data),
        .sel(ctrl_from_rr_ex[2]), .O(rd_data_alu_or_lhi)
    );
    mux_21 #(.IN_WIDTH(16),.OUT_WIDTH(16)) pc_data_mux (
        .A(rd_data_alu_or_lhi), .B(pc_next_from_rr_ex),
        .sel(pc_data_sel_rr), .O(rd_data_at_ex)
    );
    ...
    always @(*) begin
        ...
        if (ir_from_rr_ex[15:12] == 4'b1000) begin  // BEQ
            if (alu_src_A == RF_D2_from_rr_ex) begin
                flush_from_ex   = 1'b1;
                pc_ctrl_ex      = 2'b10;
            ...
            flush_from_ex   = 1'b1;
            pc_ctrl_ex      = 2'b10;
            pc_next_from_ex = alu_src_A +
                              {{7{ir_from_rr_ex[8]}}, ir_from_rr_ex[8:0]};
        end
    end

    // NOP inject for EX stage on load-use
    wire reg_wr_en_ex_out  = nop_inject ? 1'b0 : reg_wr_en_final;
    wire mem_wr_en_ex_out  = nop_inject ? 1'b0 : ctrl_from_rr_ex[0];
    assign new_ctrl_ex = {reg_wr_en_ex_out, ctrl_from_rr_ex[1], mem_wr_en_ex_out};
    ...
    // Debug memory read
    data_memory dmem_debug (
        .clk(clk), .rst(reset),
        .mem_access_addr({10'b0, debug_mem_addr}),
        .mem_write_data(16'd0),
        .mem_write_en(1'b0),
        .mem_read_data(debug_mem_data)
    );
    ...
    // =========================================================================
    always @(posedge clk) begin
        $display("WB: we=%b rd=%0d data=%h ctrl_exmm=%b",
        reg_wr_en_at_wb, rd_addr_at_wb, rd_data_at_wb, ctrl_from_ex_mm);
    end
